...
const projects = [
  {
    name: "SkillConnect",
    description:
      "A skills-first platform that helps students discover roles matched to their strengths.",
    tech: ["Next.js", "TypeScript", "Tailwind"],
    role: "Product engineer",
    outcome: "Built the student dashboard and opportunity matching flow.",
    color: "#e9f0ff",
    mark: "S",
  },
  {
    name: "TimeTable Generator System",
    description:
      "A constraint-based web tool that creates clash-free schedules for college departments.",
    tech: ["React", "Node.js", "PostgreSQL"],
    role: "Frontend developer",
    outcome: "Reduced manual timetable planning time for faculty by 60%.",
    color: "#e3f2e9",
    mark: "T",
  },
  {
    name: "LocalCart",
    description:
      "An accessible e-commerce experience for discovering products from neighbourhood stores.",
    tech: ["JavaScript", "SQL", "CSS"],
    role: "Full-stack developer",
    outcome: "Designed checkout flows and shipped the first responsive MVP.",
    color: "#fff0d8",
    mark: "L",
  },
];
...
const certifications = [
  {
    name: "Frontend Development Foundations",
    issuer: "SkillConnect Assessments",
    date: "18 Aug 2026",
...
function SectionHeading({
  ...
  return (
    <div className="mb-5">
      ...
      <h2 className="mt-1 font-display text-[1.45rem] font-semibold tracking-[-0.03em]">
        {title}
      </h2>
    </div>
  );
  ...
  return (
    <Shell menuOpen={menuOpen} setMenuOpen={setMenuOpen}>
      <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-2 font-display text-[2.25rem] font-semibold leading-[1.05] tracking-[-0.055em] sm:text-[2.75rem]">
            Your portfolio <span className="text-coral">.</span>
          </h1>
          <p className="mt-3 text-[0.95rem] text-muted">
            Showcase what you can do, not just what you&apos;ve studied.
          </p>
        </div>
      ...
      <section className="rounded-2xl bg-ink p-6 text-white sm:p-8">
        <div className="flex flex-col gap-7 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4 sm:gap-6">
            <div className="grid h-20 w-20 shrink-0 place-items-center rounded-2xl bg-[#d9d4c8] font-display text-2xl font-bold text-ink sm:h-24 sm:w-24">
              AS
            </div>
            <div>
              <p className="eyebrow text-[#b3b8b0]">Open to opportunities</p>
              <h2 className="mt-2 font-display text-2xl font-semibold tracking-[-0.04em] sm:text-3xl">
                Aarav Sharma
              </h2>
              <p className="mt-1 text-sm text-[#d0d5cd]">
                BSc Information Technology
              </p>
              <p className="mt-2 flex items-center gap-1.5 text-sm text-[#b3b8b0]">
                <MapPin size={14} />
                {location}
              </p>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-[#b3b8b0]">
                {summary}
              </p>
            </div>
          </div>
          <div className="w-full shrink-0 border-t border-white/10 pt-5 lg:w-56 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold">Profile completeness</p>
              <p className="font-display text-2xl font-bold text-coral">82%</p>
            </div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/15">
              <div className="h-full w-[82%] rounded-full bg-coral" />
            </div>
            <p className="mt-2 text-xs text-[#b3b8b0]">
              Add one more project to reach 90%.
            </p>
            <button
              onClick={sharePortfolio}
              className="mt-5 flex items-center gap-2 text-sm font-bold text-white hover:text-coral"
            >
      ...
      <div className="mt-10 grid gap-8 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {skills.map((skill) => (
              ...
              >
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#eeeee9]">
                  <div
        ...
        <section>
          <SectionHeading eyebrow="Where I learned it" title="Education" />
          <article className="rounded-2xl border border-line bg-white p-5 sm:p-6">
            <div className="flex gap-4">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#e9f0e8] text-olive">
      ...
      <section className="mt-10">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project) => (
            <article
              key={project.name}
              className="flex flex-col rounded-2xl border border-line bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(35,43,38,0.08)]"
            >
              ...
              <p className="mt-5 border-t border-line pt-4 text-xs leading-5 text-muted">
                <span className="font-bold text-ink">Key contribution: </span>
                {project.outcome}
              </p>
              <div className="mt-auto flex gap-4 pt-5">
                <a
                  href={`https://github.com/aarav-sharma/${project.name.toLowerCase().replaceAll(" ", "-")}`}
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-1.5 text-xs font-bold text-ink hover:text-coral"
                >
                  <Github size={15} />
                  GitHub
                </a>
          ...
          <div className="space-y-3">
            {certifications.map((certification) => (
              <article
                ...
                className="flex items-center gap-4 rounded-2xl border border-line bg-white p-4 sm:p-5"
              >
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#fff0d8] text-[#9d6d00]">
                  <Check size={18} />
                </span>
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-6 text-muted">
                  ...
                  and improved page performance across the student portal.
                </p>
              </div>
            </div>
          </article>
      ...
      <div className="mt-10 grid gap-8 xl:grid-cols-[1fr_0.8fr]">
        <section>
          <SectionHeading eyebrow="Ready when you are" title="Resume" />
          <article className="flex flex-col gap-5 rounded-2xl border border-line bg-white p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div className="flex items-center gap-4">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-[#f9e9e9] text-coral">
                <FileText size={21} />
              </span>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <label className="inline-flex h-8 cursor-pointer items-center gap-2 rounded-xl bg-ink px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#3b4740]">
                <Upload size={14} />
                Update
                <input
                  type="file"
        ...
        <section>
          <SectionHeading eyebrow="Your control" title="Profile visibility" />
          <article className="rounded-2xl border border-line bg-white p-5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="font-semibold">Visible to industry</h3>
                <p className="mt-1 text-xs leading-5 text-muted">
                  Recruiters can discover your skills and projects.
                </p>
              </div>
              <button
                role="switch"
                ...
                className={`relative h-6 w-11 shrink-0 rounded-full transition ${visible ? "bg-olive" : "bg-[#cdd2cb]"}`}
              >
                <span
                  className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${visible ? "left-6" : "left-1"}`}
                />
              </button>
            </div>
            <p className="mt-5 flex items-center gap-1.5 border-t border-line pt-4 text-xs text-muted">
              <CalendarDays size={14} />
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Keep your professional profile current for industry teams.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <label className="block text-sm font-semibold">
              ...
              <textarea
                ...
                className="mt-2 min-h-24 w-full rounded-xl border border-line p-3 text-sm font-normal outline-none focus:border-coral"
              />
            </label>
            <label className="block text-sm font-semibold">
              Current location
              <input
                ...
                className="mt-2 h-10 w-full rounded-xl border border-line px-3 text-sm font-normal outline-none focus:border-coral"
              />
            </label>
          </div>
          <DialogFooter>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Public portfolio preview</DialogDescription>
          </DialogHeader>
          <div className="rounded-xl bg-[#f8f8f5] p-5">
            <p className="eyebrow">Frontend developer</p>
            <p className="mt-3 text-sm leading-6 text-muted">
              Frontend-focused IT student building practical web products with
