...
const applications: Application[] = [
  {
    ...
    mark: "R",
    timeline: [
      { label: "Applied", date: "28 Aug", complete: true },
  ...
  },
  {
    ...
    mark: "M",
    timeline: [
      { label: "Applied", date: "25 Aug", complete: true },
      { label: "Under Review", date: "29 Aug", complete: true },
      { label: "Shortlisted", date: "", complete: false },
      { label: "Interview", date: "", complete: false },
      { label: "Selected", date: "", complete: false },
    ],
  },
  {
    ...
    mark: "Z",
    timeline: [
      { label: "Applied", date: "22 Aug", complete: true },
  ...
  },
  {
    ...
    mark: "M",
    timeline: [
      { label: "Applied", date: "18 Aug", complete: true },
      { label: "Under Review", date: "", complete: false },
      { label: "Shortlisted", date: "", complete: false },
      { label: "Interview", date: "", complete: false },
      { label: "Selected", date: "", complete: false },
    ],
  },
  {
    ...
    match: 75,
    status: "Selected",
    skills: ["SQL", "Communication", "Excel"],
    missing: [],
    color: "#f9e9e9",
    mark: "D",
    timeline: [
      { label: "Applied", date: "12 Aug", complete: true },
  ...
  },
  {
    ...
    mark: "M",
    timeline: [
      { label: "Applied", date: "07 Aug", complete: true },
      { label: "Under Review", date: "10 Aug", complete: true },
      { label: "Rejected", date: "16 Aug", complete: true },
    ],
  },
];
...
function ApplicationCard({
  ...
  return (
    <article className="group rounded-2xl border border-line bg-white p-5 transition duration-300 hover:-translate-y-0.5 hover:border-[#d4d8cf] hover:shadow-[0_12px_30px_rgba(35,43,38,0.08)]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-1 truncate font-display text-lg font-semibold tracking-[-0.03em]">
              {application.role}
            </h2>
          </div>
        </div>
        <p className="shrink-0 font-display text-xl font-bold text-olive">
          {application.match}%
          <span className="block text-right text-[0.58rem] font-medium uppercase tracking-[0.12em] text-muted">
            match
          </span>
        </p>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        {canWithdraw && (
          <button className="ml-auto text-xs font-semibold text-muted transition hover:text-coral">
            Withdraw
          </button>
        )}
      </div>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <p className="mt-2 font-semibold">{application.company}</p>
        ...
        <div>
          ...
          <p className="mt-2 font-display text-2xl font-bold text-olive">
            {application.match}%
          </p>
        </div>
      </div>
      <div className="border-t border-line pt-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {application.missing
            .filter((skill) => !application.skills.includes(skill))
            .map((skill) => (
              <span
                key={skill}
          ...
          <div className="flex min-w-[560px]">
            {application.timeline.map((step, index) => (
              <div key={step.label} className="relative flex-1 text-center">
                <div
                  className={`mx-auto grid h-7 w-7 place-items-center rounded-full ${step.complete ? "bg-olive text-white" : "border-2 border-[#dfe3db] bg-white text-transparent"}`}
                >
                  {step.complete ? (
                    <Check size={14} />
                  ) : (
                    <span className="h-2 w-2 rounded-full bg-[#dfe3db]" />
                  )}
                </div>
                {index < application.timeline.length - 1 && (
                  <div
                    className={`absolute left-1/2 top-3.5 h-px w-full ${application.timeline[index + 1].complete ? "bg-olive" : "bg-[#dfe3db]"}`}
                  />
                )}
              </div>
            ))}
          </div>
          <div className="mt-2 flex min-w-[560px]">
            {application.timeline.map((step) => (
              <div
                key={step.label}
                className={`flex-1 px-1 text-center text-[0.65rem] font-semibold ${step.complete ? "text-ink" : "text-muted"}`}
              >
                {step.label}
    ...
    <StudentLayout>
      <div className="mb-8">
        ...
        <h1 className="mt-2 font-display text-[2.25rem] font-semibold leading-[1.05] tracking-[-0.055em] sm:text-[2.75rem]">
          Your applications <span className="text-coral">.</span>
        </h1>
        <p className="mt-3 max-w-xl text-[0.95rem] leading-6 text-muted">
          Keep every application, update and next step organised in one place.
        </p>
      </div>
      <section className="mb-10 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        {[
          ["Total Applications", applications.length, "All your applications"],
          [
            "Under Review",
          ...
          <div
            key={label}
            className={`min-h-[122px] rounded-2xl border border-line p-4 sm:p-5 ${index === 0 ? "bg-ink text-white" : "bg-white"}`}
          >
            <p
              className={`text-xs font-semibold ${index === 0 ? "text-[#b3b8b0]" : "text-muted"}`}
            >
              {label}
            </p>
            <p
              className={`mt-5 font-display text-3xl font-semibold tracking-[-0.06em] ${index === 0 ? "text-white" : "text-ink"}`}
            >
              {value}
            </p>
            <p
              className={`mt-1 text-[0.68rem] ${index === 0 ? "text-[#b3b8b0]" : "text-muted"}`}
            >
              {note}
            </p>
          </div>
      ...
      <section>
          ...
          <div className="flex flex-wrap gap-1.5 rounded-xl border border-line bg-white p-1">
            {filters.map((item) => (
              <button
                ...
                className={`rounded-lg px-3 py-2 text-xs font-semibold transition ${filter === item ? "bg-ink text-white" : "text-muted hover:bg-[#f1f2ed] hover:text-ink"}`}
              >
                {item}
              </button>
            ))}
        ...
        {filtered.length > 0 ? (
          ...
          <div className="rounded-2xl border border-dashed border-[#cfd5cd] bg-white px-6 py-16 text-center">
            <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#e9f0e8] text-olive">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted">
              Try another company or role, or switch to a different status
              filter.
            </p>
            <button
              onClick={() => {
            ...
            >
              Clear filters <ArrowRight className="ml-1 inline" size={15} />
            </button>
          </div>
        )}
      </section>
      ...
      >
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <DialogTitle className="mt-2 font-display text-2xl tracking-[-0.04em]">
                  {selected?.role}
                </DialogTitle>
              ...
              <DialogClose asChild>
                <button
                  ...
                  className="rounded-lg p-1 text-muted hover:bg-[#f1f2ed]"
                >
                  <X size={18} />
                </button>
              </DialogClose>
          ...
          {selected && <Details application={selected} />}
          <div className="flex items-center gap-2 border-t border-line pt-4 text-xs text-muted">
            <Clock3 size={14} />
            Timeline updates are based on your latest application activity.
          </div>
        </DialogContent>
      </Dialog>
    </StudentLayout>
