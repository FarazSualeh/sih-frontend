...
import { getStoredLearningVideos, saveLearningVideos, trainingRecommendations } from '@/lib/mock-data/academician';
import type { LearningVideo, SkillGap } from '@/lib/types';
...
export default function SkillGapInsights({ gaps }: { gaps: SkillGap[] }) {
  ...
  const [videos, setVideos] = useState<LearningVideo[]>(() => getStoredLearningVideos());
            ...
            <div key={g.id} className="border-b pb-3 last:border-b-0">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-slate-500">Student proficiency: {g.proficiency}% â€¢ Industry demand: {g.demand}%</div>
                </div>

                <div className="text-right">
                  <div className="text-sm font-medium text-rose-700">{g.gap} point gap â€¢ {g.affectedStudents} students</div>
              ...
              {isOpen && (
                <div className="mt-3 rounded-md border border-sky-100 bg-sky-50 p-3">
                  <div className="mb-3">
                    ...
                    <p className="text-xs text-slate-600 mt-1">{recommendation.category} â€¢ {recommendation.difficulty} â€¢ {recommendation.duration}</p>
