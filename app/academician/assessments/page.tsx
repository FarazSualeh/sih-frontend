...
import React, { Suspense, ChangeEvent, useEffect, useMemo, useState } from 'react';
import { useSearchParams } from 'next/navigation';
import LayoutAcademician from '@/components/academician/layout-academician';
import { ASSESSMENTS_STORAGE_KEY, getStoredValue, saveStoredValue } from '@/lib/mock-data/academician';
...
function AssessmentsPageInner() {
  ...
  const [assessments, setAssessments] = useState<Assessment[]>(() => getStoredValue<Assessment[]>(ASSESSMENTS_STORAGE_KEY, initialAssessments));
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState<'All' | Assessment['status']>('All');
  const [selectedId, setSelectedId] = useState(() => getStoredValue<Assessment[]>(ASSESSMENTS_STORAGE_KEY, initialAssessments)[0]?.id ?? '');
  ...
  const initialSkill = skillParam && skillOptions.includes(skillParam) ? skillParam : 'Python';
  const [form, setForm] = useState({
    title: initialSkill !== 'Python' ? `${initialSkill} Skills Assessment` : '',
    description: '',
    skill: initialSkill,
    duration: 30,
  ...
  useEffect(() => {
    saveStoredValue(ASSESSMENTS_STORAGE_KEY, assessments);
  }, [assessments]);
                  ...
                  <div key={assessment.id} className={`rounded border p-4 ${selectedAssessment?.id === assessment.id ? 'border-sky-200 bg-sky-50' : 'border-slate-200'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">{assessment.skill} â€¢ {assessment.duration} mins</p>
                      </div>
