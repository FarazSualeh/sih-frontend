...
import LayoutIndustry from '@/components/industry/layout-industry';
...
function generateAppMatchId(studentId: string): string {
  return `app-${studentId}-${Date.now()}`;
}

function TalentMatchesPageInner() {
  const searchParams = useSearchParams();
  const oppId = searchParams.get('oppId');

  const [opportunities] = useState(() => getStoredOpportunities());
  ...
  const activeOppId = selectedOppId || (oppId && typeof oppId === 'string' ? oppId : '') || opportunities[0]?.id || '';
  const selectedOpp = opportunities.find((o) => o.id === activeOppId) || opportunities[0];
  ...
  const shortlistCandidate = (candidateName: string, studentId: string) => {
    ...
    if (existing) {
      ...
      updatedApps.push({
        id: generateAppMatchId(studentId),
        studentId,
